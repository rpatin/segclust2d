...
[segmentation/clustering](#segmentation-clustering) algorithm. This is generally
used to identify behavioural modes.

Input data are here `data.frame` (shown in the first examples), but it can also
...
different movement mode. It is a data.frame with 11 columns, with coordinates
...
`spatial_angle`). We also checked the data for missing value.
...
value, as large value may compromise speed or selection of optimal number of
segments.
...
or use the coordinates for `move`, `ltraj` and `sftraj` objects.
...
recommended if variables chosen for segmentation are of different nature (e.g.
speed and turning angle). It is therefore inadvisable for segmentation of
coordinates to identify home-ranges. By default `segmentation()` disable
rescaling and sets `scale.variable = FALSE`.

### Optional: `diag.var`

`segmentation()` use variables provided in `seg.var` and automatically produces
...
`order.var` through which variables segments should be ordered when asked so in
some of the output functions
...
From version 0.3.0 and forward, those argument were removed so that the
...
segmentation given a number of segment. The function runs the dynamic
programming for each number of segment `<Kmax`. As a result, the optimal
...
show several examples of running `segmentation()` with different options.
Section [Selecting the number of segments](#selecting-the-number-of-segments)
discuss about the selection of the number of segments using either Lavielle's
...
function. Minimal call should contains the data concerned (here the dataframe
`simulshift`) and argument `lmin`. The function will automatically complete the
missing argument with default values and communicate about it (not shown here).
...
its own variable, depending on its aim:
...
By default, the algorithm choose the number of segment given a criterium
developed by Marc Lavielle based on the value of the second derivative of the
penalized likelihood. This criterium use a threshold value of `S = 0.75`, but a
different threshold can be specified to `segmentation()` if need be.

As stated it is important to check that the number of segment selected
correspond to a clear break in log-likelihood and if it is not the case to
...
an increasing curve with a clear breakpoints for the optimal number of segment.
Note that with real data breaks are often less clear than for that example. An
artifactual decrease of likelihood can happen for large number of segment when
Kmax is too high (close to `n/lmin`) and correspond generally to an
...
Another example on the `simulmode` dataset show a situation where the number of
segment automatically selected by Lavielle's criterium is 1, despite a
...
`segclust()` share most of its parameter with `segmentation()` so you can read
...
the number of cluster to be tested.
...
select the number of cluster, although it is advised to select it based on
...
Minimal call should contains the data concerned (here the dataframe `simulmode`)
as well as argument `lmin` and `ncluster` and `seg.var`. The function will
automatically complete the missing argument with default values and communicate
...
As stated, the number of cluster should preferentially be selected based on
biological knowledge. Exploration of the BIC-based penalized log-likelihood can
also help selecting an appropriate number of cluster, with the function
`plot_BIC()`. Best-case scenario is as below, the BIC show a steep increase up
to a maximum and a slow decrease after the optimum and one number of cluster is
...
With real data it also happens that more cluster always improve the
penalized-likelihood so it will be a poor indication of the appropriate number
of cluster. Such situation may be shown with our dataset `simulmode` indeed, if
we allow the number of cluster to be tested between 2 and 5 instead of a maximum
at three, we obtain the following BIC curve:
...
In the curve above we see that the optimum number of cluster selected is 4,
despite the difference being relatively small compared to `ncluster = 3` and to
the evolution with the number of segment. In addition, if we look at the results
of the segmentation we can see identical segment distribution, the only
difference is that state 2 in the segmentation with 3 cluster is divided into
state 2 and 3 in the segmentation with 4 cluster:
...
More generally, as in this example, if the selected number of segment for a
higher number of cluster is the same, then the lower number of cluster should be
favored.

Once the number of cluster have been selected, the number of segment should be
