...
Input data are here `data.frame` (shown in the first examples), but it can also be a `Move` object, a `ltraj` object (from package `adehabitatLT`) or a `sftraj` object, both shown in section [Preparing data for Segmentation/Clustering with segclust2d](https://rpatin.github.io/segclust2d/articles/preparing_data.html)
...
`order.var` through which variables segments should be ordered when asked so in some of the output functions (see section [Exploring Outputs from segclust2d](https://rpatin.github.io/segclust2d/articles/explore_outputs.html) for details.). By default `order.var` is set to `seg.var[1]`
...
`segmentation()` uses a Dynamic Programming algorithm that finds the best segmentation given a number of segment. The function runs the dynamic programming for each number of segment `<Kmax`. As a result, the optimal segmentation is associated with a likelihood value for each number of segment. Section [Different running configurations](#different-running-configurations) show several exemples of running `segmentation()` with different options. Section [Selecting the number of segments](#selecting-the-number-of-segments) discuss about the selection of the number of segments using either Lavielle's criterium or graphical explorations.

### Different running configurations

In this section we will try different ways of calling the `segmentation()` function. Minimal call should countains the data concerned (here the dataframe `simulshift`) and argument `lmin`. The function will automatically complete the missing argument with default values and communicate about it (not shown here). 

*`subsample_by` argument is described in section [Subsampling](https://rpatin.github.io/segclust2d/articles/preparing_data.html#subsampling)*
...
Another exemple on the `simulmode` dataset show a situation where the number of segment automatically selected by Lavielle's criterium is 1, despite a (relatively) clear break in the likelihood at `nseg = 15`. In such case it is of paramount importance to explore the log-likelihood to confirm or select an appropriate number of segments.
