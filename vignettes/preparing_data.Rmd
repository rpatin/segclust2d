...
- [Move](#move) objects based on package [`move`](https://cran.r-project.org/web/packages/move/index.html)
- [ltraj](#ltraj) objects based on package [`adehabitatLT`](https://cran.r-project.org/web/packages/adehabitatLT/index.html)
...
`Move` object can alternatively provided to the function. If using `segmentation()`, the user may omit `seg.var` argument and the algorithm will use the movement coordinates as segmentation variables. Alternatively if the user specify the segmented variable with argument `seg.var`, those variables must be present in the data associated to the `Move` object `x_move@data`
...
`ltraj` object can alternatively provided to the function. If using `segmentation()`, the user may omit `seg.var` argument and the algorithm will use the movement coordinates as segmentation variables. Alternatively if the user specify the segmented variable with argument `seg.var`, those variables must be present in the data associated to the `ltraj` object `x_ltraj@data`
...
```{r lmin and subsampling, include = FALSE}
lmin <-  240
subsample_by  <-  60
...
In addition to reducing computation time, subsampling may also help the algorithm. Considering movement at the scale of hours when looking for home-ranges at the scale of months may blur the signal and for such analysis, one data per day may be sufficient. For all analysis, the user should think about the appropriate temporal resolution, with the idea that the finest temporal resolution may not always be appropriate.
...
When pre-processing movement data before segmentation/clustering it is common to use interpolation of trajectory to correct for missing data points. This may however cause problem if this lead to repetition of values. This can also arise if the individual have a very stable speed (i.e. a boat or a bird deriving on the sea) leading to very similar values.

When the repetition of identical or very similar values are longer than parameter `lmin`, there are segments with null variance, which cannot be accounted for by the algorithm. Should such cases arises, the algorithm will not fail and tell you about it:
