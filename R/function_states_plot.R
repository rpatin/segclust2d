...
#' @param outputs outputs of the segmentation  or segclust algorithm for one number of segment
...
plot_states <- function(outputs,diag.var, position_width=0.3,order = F){
  ...
  if(class(mu.list)== 'data.frame'){
    g <- ggplot2::ggplot(data=mu.list, ggplot2::aes_string(x=state_variable,y="mu"))+
      ...
      ggplot2::geom_errorbar(ggplot2::aes_string(ymin="mu-sd",ymax="mu+sd"),width=0.1)+
      ggplot2::xlab("State")+
    ...
    g <- ggplot2::ggplot(data=mu.merged, ggplot2::aes_string(x=state_variable,y="mu",col="model"))+
      ...
      ggplot2::facet_wrap(~variable,scale="free")+
      ggplot2::geom_errorbar(ggplot2::aes_string(ymin="mu-sd",ymax="mu+sd"),width=0.1,position=ggplot2::position_dodge(width=position_width))+
      ggplot2::xlab("State")+
      ggplot2::ylab("Distribution (mean +/- sd)")
