...
By default `segclust()` will standardized variables, but one can change this by setting `scale.variable = FALSE`. 

``` r
mode_segclust <- segclust(simulmode, Kmax = 30, lmin=5, ncluster = c(2,3,4), type = "behavior", seg.var = c("speed","abs_spatial_angle"))
...
`segmentation-class` also provides methods for plotting results of segmentations. All plot methods use `ggplot2` package.
